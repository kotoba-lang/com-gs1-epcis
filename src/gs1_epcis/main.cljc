(ns gs1_epcis.main
  "Kotodama WASM entrypoint for the GS1 EPCIS clean-room actor (L4) — Clojure port.
  ...
  facts (`gs1_epcis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gs1_epcis")
(def tier "L4")
...
(def entity-specs
  [{:entity "EPCISEvent"   :plural "epcisevents"   :id-prefix "gs1epcis_epc"
    :fields [:eventId :eventType :eventTime :recordTime :eventTimeZoneOffset :certificationInfo] :required [:eventType :eventTime]
    :coerce {}                       :refs {}}
   {:entity "ObjectEvent"      :plural "objectevents"      :id-prefix "gs1epcis_obj"
    :fields [:eventId :action :bizStep :disposition :readPoint :bizLocation :eventTime :recordTime] :required [:action :bizStep]
    :coerce {}          :refs {}}
   {:entity "AggregationEvent"     :plural "aggregationevents"     :id-prefix "gs1epcis_agg"
    :fields [:eventId :parentID :action :bizStep :disposition :readPoint :eventTime]        :required [:parentID :action]
    :coerce {} :refs {}}
   {:entity "TransactionEvent" :plural "transactionevents" :id-prefix "gs1epcis_tra"
    :fields [:eventId :parentID :action :bizStep :eventTime]  :required [:parentID :action]
    :coerce {}          :refs {}}
   {:entity "TransformationEvent"    :plural "transformationevents"    :id-prefix "gs1epcis_tra"
    :fields [:eventId :transformationID :bizStep :disposition :readPoint :eventTime]  :required [:transformationID :bizStep]
    :coerce {}          :refs {}}
   {:entity "AssociationEvent"   :plural "associationevents"   :id-prefix "gs1epcis_ass"
    :fields [:eventId :parentID :action :bizStep :eventTime]   :required [:parentID :action]
    :coerce {}        :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gs1_epcis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gs1_epcis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gs1_epcis-compat/ready)
